...
type RedisRaftReconciler struct {
	client.Client
	Scheme *runtime.Scheme
}

//+kubebuilder:rbac:groups=db.tsurai.dev,resources=redisrafts,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=db.tsurai.dev,resources=redisrafts/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=db.tsurai.dev,resources=redisrafts/finalizers,verbs=update

// Reconcile is part of the main kubernetes reconciliation loop which aims to
// move the current state of the cluster closer to the desired state.
// TODO(user): Modify the Reconcile function to compare the state specified by
// the RedisRaft object against the actual cluster state, and then
...
func (r *RedisRaftReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&dbv1alpha1.RedisRaft{}).
		Complete(r)
}
